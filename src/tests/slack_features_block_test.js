  ...
  describe("/ask_block", () => {
    beforeEach(() => {
      ...
      this.controller.axiosMockAdapter
        .onPost("auth.test", { token: "sometoken" })
        .reply(200, {
          ok: true
        });
    });
    ...
    it(`/ask-block should reject when trying to block invalid user`, async () => {
      ...
      assert.strictEqual(
        reply.text,
        "Unable to block fasdfsdafasf. Invalid user."
      );
      assert.strictEqual(
        reply.channelData.response_type,
        "ephemeral",
    ...
    it(`/ask-block should block 1 person`, async () => {
      ...
      UserMock.expects("findOneAndUpdate").withArgs(
        { user_id: "UNBU4TU7L" },
        { is_banned: true }
      );
    ...
    it(`/ask-block should try to block multiple people but one is invalid`, async () => {
      ...
      const text =
        "<@UNBU4TU7L|user1> <@UNBU4TU7O|user2> fadsfasdfasdfsadfsdfsf";
      const response_url = "response_url/public";
      await this.controller.usersInput([
        {
          type: "slash_command",
      ...
      assert.strictEqual(
        reply0.text,
        "Unable to block fadsfasdfasdfsadfsdfsf. Invalid user."
      );
